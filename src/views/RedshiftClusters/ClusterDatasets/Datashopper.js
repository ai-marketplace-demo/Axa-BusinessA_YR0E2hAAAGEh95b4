...
const Datashopper = (props) => {
    ...
    const actionFormatter = (cell, row) => <div onClick={() => { linkDataset(row); }} className={'btn btn-primary btn-sm '}>Link</div>;
    ...
    const fetchItems = async () => {
        const response = await client
            .query(listAvailableDatasets({
                clusterUri: props.cluster.clusterUri,
                filter: {
                    page: datasets.page,
                    pageSize: 10,
                    term
        ...
        if (!response.errors) {
            setDatasets(response.data.listRedshiftClusterAvailableDatasets);
        } else {
    ...
    return (
        <Container>
            <Row className={'mt-4'}>
                <Col xs={1}>
                    <Link style={{ color: 'black' }} to={`/redshiftcluster/${props.cluster.clusterUri}/datasets`}>
                        <Icon.ChevronLeft size={32} />
                    </Link>
                </Col>
                <Col xs={8}>
                    <h4> Link new datasets to <b className={'text-primary text-capitalize'}>{props.cluster.label}</b> </h4>
                </Col>
            </Row>
            <Row className={'mt-4'}>
                <Col xs={12}>
