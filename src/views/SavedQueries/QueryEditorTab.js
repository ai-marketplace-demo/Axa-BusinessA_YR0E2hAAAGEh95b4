...
const QueryEditorTab = (props) => {
    ...
    const moveUp = async () => {
        ...
        setQuery({
            ...
            queries: {
                ...query.queries,
                nodes: query.queries.nodes.map((q) => {
                ...
                <If condition={!leftPanelCollapsed}>
                    <Then>
                        <Col className={'border-right'} xs={3}>
                            ...
                            {((query && query.queries && query.queries.nodes) || []).sort((e1, e2) => e1.queryOrder > e2.queryOrder).map((queryItem, index) => (
                                <Col className={'mt-2'}>
