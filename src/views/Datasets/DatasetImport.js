...
const DatasetImport = () => {
    ...
    const onSubmit = async ({formData, success, fail}) => {
        ...
        if (!formData.bucketName)
            errors.push('Amazon S3 bucket name is mandatory')
        ...
        if (errors.length > 0){
            ...
            const response = await client.mutate(importDataset({
            ...
            if (!response.errors) {
                success({
                    header: `Success`,
                    content: `Successfully imported dataset ${response.data.importDataset.datasetUri}`
                })
            } else {
                fail({
                    header: "Failed",
                    content: `Failed to import dataset, received ${response.errors[0].message}`
                })
            }
        }
    }
    ...
    return <div>
        ...
        ready={ready}
        onSubmit={onSubmit}
        breadcrumbs={`| catalog/contribute/datasets/import`}
