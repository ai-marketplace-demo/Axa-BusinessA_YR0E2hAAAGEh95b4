...
import listProjectDatasets from '../../../api/Project/listProjectDatasets';
import removeDatasetToProject from '../../../api/Project/removeDatasetToProject';
import DatashopperItem from './DatashopperItem';
...
const ProjectDatasetList = (props) => {
    ...
    const actionFormatter = (cell, row) => <div onClick={() => { unlinkDataset(row.datasetUri); }} className={'btn btn-secondary btn-sm '}>Unlink</div>;
    ...
    const unlinkDataset = async (datasetUri) => {
        toast(`Trying to unlink dataset ${datasetUri} from project ${projectUri}`);

        const res = await client.mutate(removeDatasetToProject({
            projectUri,
            datasetUri
        }));
        if (!res.errors) {
    ...
    const fetchItems = async () => {
        ...
        if (!response.errors) {
            setDatasets(response.data.listProjectDatasets);
        } else {
    ...
    return (
        <Container>
            <Row>
                <Col xs={8}>
                    <h4><Icon.Folder size={32} /> Datasets Available in this Project</h4>
                </Col>
                <Col xs={4}>
                    <Link to={'newprojectitem'}>
            ...
            <Styled>
                <Row className={'mt-4'}>
                    <If condition={datasets.count}>
                        ...
                        <Else>
                            <Col xs={12}>
                                <i>No datasets added to your project. Start linking datasets
                                    <Link to={'newprojectitem'}> Here</Link>, or create a new one <Link to={'/newdataset'}>Here</Link>
...
export default ProjectDatasetList;
