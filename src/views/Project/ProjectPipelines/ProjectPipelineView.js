...
const PipelineView  = (props)=>{
    ...
    let [currentFolder, setCurrentFolder] = useState({label:'', nodeUri:null,nodeType:'Folder'});
    let [previousFolder, setPreviousFolder] = useState({label:'', nodeUri:null,nodeType:'Folder'});
    ...
    const pop=()=>{
        if (folderHistory.length){
            folderHistory.pop();
            setFolderHistory(folderHistory.filter((t,i)=>{return i<folderHistory.length}));
        }
    }
            ...
            <Col xs={2}>
                <If condition={folderHistory.length>=1}>
                    <Then>
                        ...
                        <b className={`text-primary`}>{folderHistory.length?folderHistory.map((f)=>{return f.label}).join("/"):""}</b>
                    </Then>
                </If>
            </Col>
