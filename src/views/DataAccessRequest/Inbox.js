...
const Inbox= (props)=>{
    ...
    const fetchItems=async ()=>{
        ...
        if (!response.errors){
            setShareObjects({...response.data.requestsToMe})
        }else {
            toast(`Could not retrieve inbox message, received ${response.errors[0].message}`)
        }
    }
    ...
    return <Container>
        <Row>
            <Col xs={12}>
                <h5 ><b className={`text-primary`}>Inbox</b> Data Access Requests Sent To Me</h5>
            </Col>
        </Row>
        <Row className={`mt-3`}>
            <Col xs={3}>
