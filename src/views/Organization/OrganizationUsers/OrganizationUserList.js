...
const OrganizationUserList = (props) => {
    ...
    const [sortCriterias, setSortCriterias] = useState({ userName: 'asc', created: 'desc' });
    ...
    const fetchItems = async () => {
        ...
        if (!response.errors) {
            ...
            const role = response.data.getOrganization.userRoleInOrganization;
    ...
    const handleLeave = async ({ userName }) => {
        const response = await client.mutate(removeUser({
            input: {
