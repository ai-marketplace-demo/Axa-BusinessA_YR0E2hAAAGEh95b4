...
import {
    Container, Row, Col, Badge, Spinner, Alert
} from 'react-bootstrap';
import * as Icon from 'react-bootstrap-icons';
import {
    If, Then, Else, Case, Default
} from 'react-if';
...
const DatasetQualityRulesList = (props) => {
    ...
    const [editedRuleUri, setEditedRuleUri] = useState('');
    ...
    const [jobRun, setJobRun] = useState(undefined);
    ...
    const columns = [
        {
            ...
            formatter: (cell, row) => (
                <Link className={'text-info'} onClick={() => { setEditedRuleUri(row.ruleUri); setMode('edit'); }}>
                    {cell}
                </Link>
            )
        },
        ...
        },
        {
            ...
            headerStyle: { width: '2ch' },
            text: 'Status',
            formatter: (cell, row) => <RulesBadge status={cell} />,
        },
        {
            dataField: 'logs',
            headerStyle: { width: '5ch' },
